...
**机制**：node 重连后第一帧发送 `event=hello`，携带本地配置版本号（如 user_inbounds 的 max(updated_at)）。Server 比对 DB 当前版本，如果落后则主动 `Call("ApplyNodeUsers")` 推送完整配置。

这等价于 node 重连时的 self-sync，无需在 server 侧维护离线指令队列。
...
| 大规模节点心跳压力 | 单 server 实例支持的活跃连接上限需压测；预留心跳间隔/keepalive 调参 |
