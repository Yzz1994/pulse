...
// findLocalProxyPort 从 xray 运行配置中找到第一个 HTTP/SOCKS 入站，
...
func findLocalProxyPort(configJSON string) string {
	...
	var cfg struct {
		Inbounds []struct {
			Protocol string `json:"protocol"` // xray 字段
			Port     int    `json:"port"`     // xray 字段
		} `json:"inbounds"`
	}
