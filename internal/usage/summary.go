...
type Summary struct {
	...
	// 流量（节点维度：实际带宽，不含倍率，不受用户流量重置影响）
	TotalUploadBytes   int64 `json:"total_upload_bytes"`
	TotalDownloadBytes int64 `json:"total_download_bytes"`
	TotalUsedBytes     int64 `json:"total_used_bytes"`

	// 流量（用户维度：计费流量，含倍率，随用户流量重置归零）
	TotalBilledUploadBytes   int64 `json:"total_billed_upload_bytes"`
	TotalBilledDownloadBytes int64 `json:"total_billed_download_bytes"`
	TotalBilledUsedBytes     int64 `json:"total_billed_used_bytes"`
	...
	// 今日各节点流量分布（来自 daily_usage，当日切片）
	TodayNodeStats []NodePeriodStat `json:"today_node_stats"`
	// 今日各用户流量排行（Top 20，来自 user_node_daily_usage）
	TodayUserStats []users.TodayUserStat `json:"today_user_stats"`
...
func Build(nodeStore nodes.Store, userStore users.Store, days int) (Summary, error) {
	...
	for _, u := range usersList {
		...
		s.TotalBilledUploadBytes += u.UploadBytes
		s.TotalBilledDownloadBytes += u.DownloadBytes
	...
	s.TotalBilledUsedBytes = s.TotalBilledUploadBytes + s.TotalBilledDownloadBytes
	...
	var todayRaw []nodes.NodeDailyUsage
	for _, r := range dailyRaw {
		if r.Date == today {
			todayRaw = append(todayRaw, r)
		}
	}
	s.TodayNodeStats = aggregateNodePeriodStats(todayRaw, nodesList)

	if todayUsers, err := userStore.ListTodayUserStats(20); err == nil {
		s.TodayUserStats = todayUsers
	} else {
		log.Printf("usage.Build: ListTodayUserStats: %v", err)
	}
