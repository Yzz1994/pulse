...
type Config struct {
	...
	// 节点 gRPC（控制面）
	// 面板 HTTP + gRPC TLS 共用 ServerAddr（cmux 按 TLS ClientHello 分流）。
	// Cloudflare 需配置为 Flexible 模式（HTTP 到源站），节点直连走 TLS。
	NodeGRPCURL string // PULSE_NODE_GRPC_URL，返回给节点的外部连接地址
